        ...
        <div className="flex flex-wrap items-center justify-between gap-6 bg-[#141414] p-3 rounded-2xl border border-[#2A2A2A] sticky top-6 z-50">
          <div className="flex items-center gap-6 px-4">
            <div className="flex items-center gap-3">
              <span className="font-bold tracking-tight text-xl text-white">KIRAMMA</span>
            </div>
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide p-1 bg-[#0B0B0B] rounded-xl border border-[#2A2A2A]">
            ...
            <button onClick={() => setMode('personal')} className={cn("pill-tab", mode === 'personal' && "active")}>
              <LayoutGrid size={15} /> 
                      ...
                      <AnimatePresence mode="wait">
                        {isRealtimeLoading ? (
                          ...
                          >
                            ...
                            {realtimeAssistance.correctedInput && 
                             realtimeAssistance.correctedInput.toLowerCase() !== input.toLowerCase() && 
                             realtimeAssistance.correctedInput.length > 5 && (
                              <motion.div 
                                initial={{ opacity: 0, x: -10 }}
                                animate={{ opacity: 1, x: 0 }}
                                className="bg-[#1A1A1A]/95 backdrop-blur border border-[#2A2A2A] rounded-xl p-3 flex items-center justify-between gap-4 shadow-2xl"
                              >
                                <div className="flex items-center gap-3 overflow-hidden">
                                  ...
                                  <div className="min-w-0">
                                    <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest block mb-0.5">Grammar_Fix</span>
                                    <p className="text-[11px] text-[#9CA3AF] truncate italic">"{realtimeAssistance.correctedInput}"</p>
                                  </div>
                                </div>
                                <button
                                  onClick={() => setInput(realtimeAssistance.correctedInput)}
                                  className="flex-shrink-0 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-black rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all"
                                >
                                  Apply
                                </button>
                              </motion.div>
                            ...
                            {realtimeAssistance.recommendations.length > 0 && (
                              <div className="flex flex-wrap gap-2 p-2 bg-[#141414]/90 backdrop-blur border border-[#2A2A2A] rounded-xl shadow-xl">
                                <div className="w-full flex items-center gap-2 px-1 mb-1">
                                  <Sparkles size={10} className="text-[#FACC15]" />
                                  <span className="text-[9px] font-bold text-[#404040] uppercase tracking-[0.2em]">Next_Word_Suggestions</span>
                                </div>
                                {realtimeAssistance.recommendations.map((rec, idx) => (
                                  <button
                                    key={idx}
                                    onClick={() => applyRecommendation(rec)}
                                    className="px-3 py-1.5 bg-[#0B0B0B] border border-[#2A2A2A] rounded-lg text-[11px] font-medium text-[#9CA3AF] hover:bg-[#FACC15] hover:text-black hover:border-[#FACC15] transition-all flex items-center gap-1.5 group/btn"
                                  >
                                    <span className="opacity-40 group-hover/btn:opacity-100">+</span>
                                    {rec}
                                  </button>
                                ))}
                              </div>
                            ...
                            {/* Warnings/Conflict Tags */}
                            {realtimeAssistance.warnings.length > 0 && (
                              <div className="flex flex-wrap gap-2 p-2 bg-rose-500/5 backdrop-blur border border-rose-500/20 rounded-xl">
                                {realtimeAssistance.warnings.map((warning, idx) => (
                                  <div key={idx} className="flex items-center gap-2 px-2 py-1 text-[10px] text-rose-400 font-medium">
                                    <AlertCircle size={10} />
                                    {warning}
                                  </div>
                                ))}
                          ...
                          <div className="flex items-center justify-center h-full">
                            <span className="text-[10px] font-mono text-[#2A2A2A] uppercase tracking-widest">Realtime_Assistance_Idle</span>
                          </div>
                        )}
                      </AnimatePresence>
                    </div>
